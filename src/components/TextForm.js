...
export default function TextForm(props) {
  let [heading, setHeading] = useState(
    "TextConverter - Get Your Text Formated!!!"
  );
  ...
  const handleUpClick = () => {
    ...
    setTimeout(() => {
      setHeading("TextConverter - Get Your Text Formated!!!");
    }, 400);
  };
  ...
  const handleClearClick = () => {
    let newText = "";
    setText(newText);
    props.showAlert("Text Cleared!", "done");
  ...
  const handleExtraSpaces = () => {
    ...
    props.showAlert("Extra spaces removed!", "done");
  ...
  const handleCut = () => {
    navigator.clipboard.writeText(text);
    setText("");
    props.showAlert("Copy and Removed!", "done");
  ...
  return (
    <>
      ...
      >
        ...
        <h2>Preview</h2>
        <p>{text.length > 0 ? text : "you didn't write text yet!"}</p>
      </div>
    </>
  );
}
